...
function render({ model, el }) {
  ...
  const posterior_hierarchy = model.get("hierarchy");
  ...
  data_variables_select.addEventListener(
    "change",
  ...
  function update_plot() {
    ...
    for (let chain_num = 0; chain_num < num_chains; chain_num++) {
      ...
      let y =
        posterior_data[selected_data_variable][selected_dimension_variable][
          "chains"
        ][`chain${chain_num} `];
...
function determine_posterior_hierarchy(posterior) {
  ...
  for (let data_variable_name in data_variables_object) {
    const data_dimensions_array =
      posterior["data_vars"][data_variable_name]["dims"];
    ...
    for (let data_dimension_name of data_dimensions_array) {
      ...
      const dimension_values_array =
        posterior["coords"][data_dimension_name]["data"];
      posterior_hierarchy_array.push([
        data_variable_name,
        {
          dimension_name: data_dimension_name,
          dimension_values: dimension_values_array,
        },
      ]);
    }
  }
  const posterior_hierarchy_object = Object.fromEntries(
    posterior_hierarchy_array,
  );
...
function parse_posterior_data(posterior) {
  ...
  for (let data_variable_name in posterior_hierarchy) {
    ...
    for (let i = 0; i < num_chains; i++) {
      ...
      const data_dimension_names =
        posterior_hierarchy[data_variable_name]["dimension_values"];
      if (is_hierarchical) {
...
function compute_forestplot_data(posterior_data, posterior_hierarchy) {
  ...
  for (let data_variable in posterior_data) {
    ...
    if (data_dimensions.length === 0) {
      ...
      const mean =
        all_chain_data.reduce((previous, current) => previous + current, 0) /
        all_chain_data.length;
      let datum = {
        name: data_variable,
    ...
    if (data_dimensions.length != 0) {
      for (let data_dimension of data_dimensions) {
        const chain_data_array =
          posterior_data[data_variable][data_dimension]["chains"];
        let all_chain_data: number[] = new Array();
        for (let chain_data of chain_data_array) {
          all_chain_data.push(...chain_data);
        }
