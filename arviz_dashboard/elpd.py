...
def dashboard_elpd(idatas_cmp):
    ...
    thre_slider = pn.widgets.FloatSlider(name='Threshold Value', start=1, end=3,step=0.1,value=2, width=200)
    thre_sel= pn.widgets.Select(value=None, options = list(["standard deviation"]) , name="Threshold Type",width=200)
    ...
    @pn.depends(model_selection1.param.value, model_selection2.param.value, 
                thre_slider.param.value,ic_group.param.value)
    def get_elpd_plot(model_selection1,model_selection2,thre_slider,ic_group):
        ...
        if model_selection1!=model_selection2:
            if old_model_select1!="" and old_model_select1==model_selection1 and old_model_select2==model_selection2:
                str_pane.object=""
                keys=[]
                #extract the chosen models from list
                keys.append(model_selection1)
                keys.append(model_selection2)
                fig, scatter_plt = plt.subplots(figsize=(15, 15))
                dict_cmp = {x:idatas_cmp[x] for x in keys}

                #if the model selection did not change, we only need to update the y_min and y_max
                scatter_plt=az.plot_elpd(dict_cmp, plot_kwargs={"marker":"."}, threshold=thre_slider,ic=ic_group)
    ...
    def get_stacked_bar_plot(model_selection1,model_selection2,thre_slider,ic_group):
        if model_selection1!=model_selection2:
            ...
            x_ob=scatter_ax.collections[0].get_offsets()[:,0]
            y_ob=scatter_ax.collections[0].get_offsets()[:,1]
            neg_count=sum(1 for y_ob_point in list(y_ob) if y_ob_point < 0)
            neg_sum=sum(abs(y_ob_point) for y_ob_point in list(y_ob) if y_ob_point < 0)
            pos_count=sum(1 for y_ob_point in list(y_ob) if y_ob_point > 0)
            pos_sum=sum(y_ob_point for y_ob_point in list(y_ob) if y_ob_point > 0)
            pos_sum_stan=pos_sum/(pos_sum+neg_sum)*100
            neg_sum_stan=neg_sum/(pos_sum+neg_sum)*100
            ...
            p = figure(x_range=standard, plot_height=300, plot_width=200, title="Summary of Observations",
    ...
    def get_compare_plot(model_selection1,model_selection2):
        if model_selection1!=model_selection2:
            ...
            compare_plt=az.plot_compare(model_compare,backend="bokeh")
    ...
    # show up
    display(pn.Column(pn.Row(model_selection1,model_selection2),
              pn.Row(thre_sel,pn.Column("Information Criterion:",ic_group)),
              thre_slider,
              str_pane,
              pn.Row(get_elpd_plot,get_stacked_bar_plot,get_compare_plot)).servable())