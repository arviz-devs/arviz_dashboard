...
def dashboard_ppc(idatas_cmp):
    ...
    thre_slider = pn.widgets.FloatSlider(name='Threshold Value', start=0, end=1,step=0.05,value=0.7, width=200)
    
    
    @pn.depends(model_selection1.param.value, thre_slider.param.value) 
    def get_ppc_plot(model_selection1,thre_slider):
        ppc_plt=az.plot_ppc(idatas_cmp[model_selection1], data_pairs={"y":"y"}, backend="bokeh", show=False, observed_rug=True)
        vars(
            vars(vars(ppc_plt[0][0])["_property_values"]["renderers"][2])["_property_values"][
                "glyph"
                ]
        ...
        for location in range(glyph_len):
            glyph=vars(vars(khat_plt)['_property_values']['renderers'][location])['_property_values']["glyph"]
    ...
    def get_khat_plot(model_selection1, thre_slider):
        ...
        khat_plt_update=az.plot_khat(loo_radon, color=colors,show_bins=True,backend="bokeh",show=False)
        # khat_plt_update.width=400
        # khat_plt_update.height=300
        return (khat_plt_update)

    # show up
    display(pn.Column(pn.Row(model_selection1),
              thre_slider,
              pn.Row(get_ppc_plot,get_khat_plot)).servable())